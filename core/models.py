...
class Product(models.Model):
    ...
    def get_percentage(self):
     if self.old_price:  # Check if old_price is not zero to avoid division by zero error
...
class CartOrderProducts(models.Model):
    order = models.ForeignKey('CartOrder', on_delete=models.CASCADE)
    product = models.ForeignKey(Product, on_delete=models.CASCADE, related_name='order_products', default=1)  # Assuming there is a product with id 1
    invoice_no = models.CharField(max_length=200)
    product_status = models.CharField(max_length=200)
    item = models.CharField(max_length=200)
    image = models.CharField(max_length=200)
...
class ProductReview(models.Model):
    ...
    def __str__(self):
        return self.product.title if self.product else f"Review - {self.pk}"
